...
`encode/httpx` was chosen for the GitHub source because a very large repo (e.g. `microsoft/vscode` with 100k+ issues for `state=all`) is impractical to fully paginate in a demo. httpx has enough issues to genuinely exercise multi-page Link-header pagination in well under a minute.

Both runs, plus an idempotency check (re-triggering `rick_and_morty_characters` with the same `trigger_key` returned the same `run_id` in ~50ms instead of re-crawling), were verified against the live APIs, not mocks.
...
**Observability.** Structured JSON logs (`app/logging_config.py`) on every fetch attempt — source, URL, status, latency, attempt number — plus `job_runs` as a queryable run-level view via the API. This is enough to see exactly what happened during a run without standing up a metrics/tracing stack.
...
- **Host Postgres port is 5433, not 5432.** Compose's `db` service is mapped to `5433:5432` on the host to avoid collisions with any pre-existing local Postgres on the default port. Only the host-side port changed; the app talks to `db:5432` inside the compose network, unaffected.
...
- **GitHub demo repo is `encode/httpx`**, not a larger repo like `microsoft/vscode`, because `state=all` on vscode is 100k+ issues — real pagination, but not a demo you'd want to sit through.
...
My role was architecture, design decisions, and verification — the parts where engineering judgment matters and AI tooling isn't self-sufficient. I scoped the brief's open questions before any code was generated (which APIs, auth strategy, hosting target, repo visibility), defined the strategy pattern and config-driven architecture, and reviewed every generated file for correctness before accepting it. The assistant accelerated implementation of well-specified components — strategy classes, Pydantic schemas, CRUD routes — but the system-level decisions (e.g. Postgres-only with no SQLite fallback, upsert-based idempotency, the `max_pages` safety cap) and the bug catches described below were mine. I also handled all environment-level concerns directly: secrets management, Docker and deployment configuration, and resolving infrastructure issues like port collisions with existing local services.
