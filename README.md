...
This project was built using Claude Code (Anthropic's CLI agent) as the implementation tool, working from a detailed project brief (`CLAUDE.md`) that the project owner wrote up front — the requirements checklist, architecture, tech-stack decisions, and phased build plan were the owner's own instructions to the assistant, not something the assistant invented. `CLAUDE.md` is intentionally not part of this repo (it's an internal brief/scratchpad for driving the build, not deliverable content), which is itself a decision the owner made and had the assistant apply retroactively after it was committed once by mistake.

The build was not unattended. Before any code was written, the brief's open questions (which two APIs to demo, whether to actually exercise a real GitHub token vs. staying unauthenticated, which hosting platform to attempt, public vs. private repo) were put to the owner explicitly and answered before the corresponding code was written. Every file the assistant generated was reviewed and approved by the owner before being accepted — including catching and requiring a fix to the commit convention (no AI co-author attribution), requiring the internal brief file be untracked from the repo, and directing real infrastructure decisions in real time: providing the GitHub token used by the live demo, installing Docker Desktop mid-build when it turned out not to be available in the assistant's environment, and confirming the Postgres port remap after a collision with a pre-existing local service.

Three concrete mistakes were caught during the build, in increasing order of severity:
...
That third case is the clearest argument in this build for why the brief requires demoing against real APIs rather than stopping at mocked tests: the feature was completely broken, with full test coverage green, until someone watched a real run.
